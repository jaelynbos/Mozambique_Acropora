...
first=1
for f in ${OUTDIR}/Acropora.N*beagle.gz; do
    if [ $first -eq 1 ]; then
        zcat $f
        first=0
    else
...
done | bgzip > ${OUTDIR}/Acropora_all.beagle.gz

first=1
for f in ${OUTDIR}/Acropora.N*.mafs.gz; do
    if [ $first -eq 1 ]; then
        zcat $f
        first=0
    else
...
done | bgzip > ${OUTDIR}/Acropora_all.mafs.gz